...
class client
{
    ...
    public function __construct($wsdl = null, $options = null)
    {
        if (!extension_loaded('soap')) {
            throw new \moodle_exception('SOAP extension is not loaded.');
        }
    ...
    public function setOptions($options)
    {
        foreach ($options as $key => $value) {
            switch ($key) {
                ...
                default:
                    throw new \moodle_exception('Unknown SOAP client option');
                    break;
            }
        }
    ...
    public function setSoapVersion($version)
    {
        if (!in_array($version, array(SOAP_1_1, SOAP_1_2))) {
            throw new \moodle_exception('Invalid soap version specified. Use SOAP_1_1 or SOAP_1_2 constants.');
        }
    ...
    public function setClassmap(array $classmap)
    {
        foreach ($classmap as $type => $class) {
            if (!class_exists($class)) {
                throw new \moodle_exception('Invalid class in class map');
            }
        }
    ...
    public function setEncoding($encoding)
    {
        if (!is_string($encoding)) {
            throw new \moodle_exception('Invalid encoding specified');
        }
    ...
    public function validateUrn($urn)
    {
        ...
        if ($scheme === false || $scheme === null) {
            throw new \moodle_exception('Invalid URN');
        }
    ...
    public function setStyle($style)
    {
        if (!in_array($style, array(SOAP_RPC, SOAP_DOCUMENT))) {
            throw new \moodle_exception('Invalid request style specified. Use SOAP_RPC or SOAP_DOCUMENT constants.');
        }
    ...
    public function setEncodingMethod($use)
    {
        if (!in_array($use, array(SOAP_ENCODED, SOAP_LITERAL))) {
            throw new \moodle_exception('Invalid message encoding method. Use SOAP_ENCODED or SOAP_LITERAL constants.');
        }
    ...
    public function setHttpsCertificate($localCert)
    {
        if (!is_readable($localCert)) {
            throw new \moodle_exception('Invalid HTTPS client certificate path.');
        }
    ...
    protected function _initSoapClientObject()
    {
        ...
        if ($wsdl == null) {
            if (!isset($options['location'])) {
                throw new \moodle_exception('\'location\' parameter is required in non-WSDL mode.');
            }
            if (!isset($options['uri'])) {
                throw new \moodle_exception('\'uri\' parameter is required in non-WSDL mode.');
            }
        } else {
            if (isset($options['use'])) {
                throw new \moodle_exception('\'use\' parameter only works in non-WSDL mode.');
            }
            if (isset($options['style'])) {
                throw new \moodle_exception('\'style\' parameter only works in non-WSDL mode.');
            }
        }
        unset($options['wsdl']);
    ...
    public function getFunctions()
    {
        if ($this->getWsdl() == null) {
            throw new \moodle_exception('\'getFunctions\' method is available only in WSDL mode.');
        }
    ...
    {
        if ($this->getWsdl() == null) {
            throw new \moodle_exception('\'getTypes\' method is available only in WSDL mode.');
        }
